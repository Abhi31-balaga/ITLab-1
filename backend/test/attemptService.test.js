...
test("submits server-scored answers and ignores frontend scoring data", () => {
  ...
  const answers = attempt.questionIds.map((questionId, index) => ({
    questionId,
    answer: index < 7 ? questions.get(questionId).correctAnswer : 0,
    correctAnswer: 999,
    score: 999,
  }));
  ...
  assert.throws(
    () =>
      submitAttempt({
        attemptId: attempt.id,
        user: { id: "student-1", role: "STUDENT" },
        answers: [{ questionId: "not-in-attempt", answer: 0 }],
        now: new Date("2026-08-17T12:10:00.000Z"),
      }),
  ...
  assert.throws(
    () =>
      submitAttempt({
        attemptId: attempt.id,
        user: { id: "student-1", role: "STUDENT" },
        answers: [{ questionId: attempt.questionIds[0], answer: 99 }],
        now: new Date("2026-08-17T12:10:00.000Z"),
      }),
    /available option/,
  );
