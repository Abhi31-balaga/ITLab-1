...
const { exams, questions, attempts, resetStore } = require('../src/data/store');
const { startAttempt, submitAttempt, getResult } = require('../src/services/attemptService');
...
test('starts an available exam with exactly ten sanitized questions', () => {
  ...
  response.questions.forEach((question) => {
    assert.deepEqual(Object.keys(question), ['id', 'text', 'options']);
    assert.equal(question.correctAnswer, undefined);
    assert.equal(question.marks, undefined);
  });
});

test('rejects starts with fewer than ten questions', () => {
  seedExam(9);
  assert.throws(
    () => startAttempt({ examId: 'exam-1', user: { id: 'student-1', role: 'CLIENT' } }),
...
test('submits server-scored answers and ignores frontend scoring data', () => {
  ...
  const answers = attempt.questionIds.map((questionId, index) => ({
    questionId,
    answerIndex: index < 7 ? questions.get(questionId).correctAnswer : 0,
    correctAnswer: 999,
    score: 999,
  }));
  ...
  assert.deepEqual(result, {
    ...
    score: 8,
    percentage: 80,
    submittedAt: '2026-08-17T12:10:00.000Z',
  });
...
test('rejects duplicate, foreign, invalid, and late submissions', () => {
  ...
  assert.throws(
    () => submitAttempt({ attemptId: attempt.id, user: { id: 'student-2', role: 'STUDENT' }, answers: [], now: new Date('2026-08-17T12:10:00.000Z') }),
    /does not belong/,
  );
  assert.throws(
    () => submitAttempt({
      attemptId: attempt.id,
      user: { id: 'student-1', role: 'STUDENT' },
      answers: [{ questionId: 'not-in-attempt', answerIndex: 0 }],
      now: new Date('2026-08-17T12:10:00.000Z'),
    }),
  ...
  assert.throws(
    () => submitAttempt({
      attemptId: attempt.id,
      user: { id: 'student-1', role: 'STUDENT' },
      answers: [{ questionId: attempt.questionIds[0], answerIndex: 99 }],
      now: new Date('2026-08-17T12:10:00.000Z'),
    }),
    /available option/,
  );
  ...
  seedExam(10);
  const { attempt: lateAttempt } = startAttempt({
    examId: 'exam-1',
    user: { id: 'student-1', role: 'STUDENT' },
    now: new Date('2026-08-17T12:00:00.000Z'),
  });
  assert.throws(
    () => submitAttempt({
      ...
      user: { id: 'student-1', role: 'STUDENT' },
      answers: [],
      now: new Date('2026-08-17T12:31:00.000Z'),
    }),
